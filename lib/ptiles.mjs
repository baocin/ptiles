...
// Keyed on the 7th magic byte, which is what distinguishes the layers.
// A layer missing here reads as 'unknown', so every published magic belongs.
export const MAGIC_LAYER = {
  0x46: 'buildings',  // PTILESF
  0x52: 'roads',      // PTILESR — also highways_v2
  0x41: 'admin',      // PTILESA — and address files built before the PTILESD fix
  0x44: 'address',    // PTILESD
  0x57: 'water',      // PTILESW
  0x50: 'places',     // PTILESP
  0x54: 'rail',       // PTILEST
  0x4e: 'parks',      // PTILESN
  0x42: 'business',   // PTILESB
  0x58: 'business_name_index',  // PTILESX
  0x43: 'camera',     // PTILESC
  0x53: 'signals',    // PTILESS
  0x55: 'routing',    // PTILESU
};

// Index entry widths. v1 is 19 bytes; v2 adds a per-cell bbox and the cell's
// index within a merged block, for 38 — not 37, which this file previously
// assumed and which silently misparsed every v2 index.
export const INDEX_ENTRY_V1 = 19;
export const INDEX_ENTRY_V2 = 38;

// Filename suffixes per layer, newest first. Published files are buildings_v9
// and business_v4; the bare names are older builds kept for compatibility.
export const LAYER_SUFFIXES = {
  ...
  address: ['address_v2', 'address_v1', 'address'],
...
// Decoders are chosen by what a file actually needs, not by what happens to be
// available. Seven of the twelve layers ship with a trained zstd dictionary,
// and neither Node's built-in zstd nor DecompressionStream can apply one:
// Node accepts a {dictionary} option and ignores it, then fails with
// "Dictionary mismatch" at decode time. Selecting on availability alone meant
// picking a decoder that could never read the file it was opened for.
let _plainZstd = null;
let _dictZstd = null;

async function _loadNodeZstd() {
  if (typeof process === 'undefined' || !process.versions?.node) return null;
  try {
    const z = await import('node:zlib');
    if (typeof z.zstdDecompressSync !== 'function') return null;
    // No dictionary support: Node exposes ZSTD_error_dictionary_wrong but no
    // way to supply a dictionary, so this is registered as plain-only.
    return { name: 'node:zlib', decompress: (c) => new Uint8Array(z.zstdDecompressSync(Buffer.from(c))) };
  } catch { return null; }
}

async function _loadStreamZstd() {
  if (typeof DecompressionStream === 'undefined') return null;
  try { new DecompressionStream('zstd'); } catch { return null; }
  return {
    ...
    decompress: async (compressed) => {
      ...
      let p = 0; for (const x of chunks) { out.set(x, p); p += x.length; }
      return out;
    },
  };
}

// The wasm module initialises once per process; init() is idempotent for us
// but re-running it while a context pointer is outstanding invalidates that
// pointer, so the module handle is cached separately from the decoder.
let _wasmMod = null;

async function _loadWasmZstd() {
  try {
    if (!_wasmMod) {
    ...
    const mod = _wasmMod;
    if (typeof mod.decompressUsingDict !== 'function') return null;
    if (typeof mod.createDCtx !== 'function') return null;
    return {
      name: '@bokuweb/zstd-wasm',
      decompress: async (c) => mod.decompress(c),
      // decompressUsingDict(dctx, buf, dict) takes a decompression context
      // first, not the buffer. Calling it as (buf, dict) leaves `dict`
      // undefined and throws inside wasm on `dict.byteLength` — how this was
      // originally written, so the dictionary path had never once worked.
      //
      // A context is created and freed per call. Holding one across calls is
      // faster but couples every decode to the lifetime of a wasm pointer,
      // which a module re-init silently invalidates; the allocation is small
      // against decompressing a block.
      decompressWithDict: async (c, dict) => {
        const dctx = mod.createDCtx();
        try {
          return mod.decompressUsingDict(dctx, c, dict);
        } finally {
          try { mod.freeDCtx(dctx); } catch { /* best effort */ }
        }
      },
    };
  } catch { return null; }
...
/**
 * Resolve a decoder able to read this file.
 * @param {boolean} needsDictionary true when the file carries a trained dictionary
 */
export async function ensureZstd(needsDictionary) {
  if (needsDictionary) {
    if (_dictZstd) return _dictZstd;
    _dictZstd = await _loadWasmZstd();
    if (!_dictZstd) {
      throw new Error(
        'This file uses a trained zstd dictionary, which needs @bokuweb/zstd-wasm. ' +
        "Node's built-in zstd and DecompressionStream both ignore the dictionary " +
        'and fail with "Dictionary mismatch". Install @bokuweb/zstd-wasm, or use a ' +
        'layer built without one (address, parks, rail, business_name_index).'
      );
    }
    return _dictZstd;
  }
  if (_plainZstd) return _plainZstd;
  _plainZstd = (await _loadNodeZstd()) || (await _loadStreamZstd()) || (await _loadWasmZstd());
  if (!_plainZstd) {
    throw new Error('No zstd available. Node 22+ has it built in; in a browser use ' +
                    'DecompressionStream("zstd") or install @bokuweb/zstd-wasm.');
  }
  return _plainZstd;
}

/** Test seam: forget cached decoders so selection can be re-exercised. */
export function _resetZstd() { _plainZstd = null; _dictZstd = null; }

/**
 * Work out the entry width from the section itself.
 *
 * The width does not follow the record-format version — the shipped v4
 * business files pair v4 records with a 19-byte v1 index, while camera and
 * signals use 38. So it is measured: the section is exactly
 * `4 + count * stride`, which pins it. An inexact match is an error rather
 * than a guess, because guessing wrong yields plausible-looking garbage
 * offsets instead of a failure.
 */
export function indexStride(byteLength, count, section = null) {
  if (count === 0) return INDEX_ENTRY_V1;
  const payload = byteLength - 4;

  // Exact fit settles it outright.
  if (payload === count * INDEX_ENTRY_V2) return INDEX_ENTRY_V2;
  if (payload === count * INDEX_ENTRY_V1) return INDEX_ENTRY_V1;

  // Otherwise the section is longer than the entries need, which is a real
  // published condition rather than corruption: US.signals and US.camera were
  // written with index_length computed at 42 bytes while the encoder emitted
  // 38, leaving 432,692 and 145,580 bytes of slack respectively. Refusing
  // those would refuse files that are on the CDN right now, so the width is
  // probed instead — the same thing the Rust client does.
  //
  // Probing is safe because index entries are sorted by h3_cell: read at the
  // wrong width and the cells stop ascending almost immediately.
  const candidates = [INDEX_ENTRY_V2, INDEX_ENTRY_V1]
    .filter(s => payload >= count * s);
  if (section) {
    for (const stride of candidates) {
      if (_entriesLookSorted(section, count, stride)) return stride;
    }
  ...
  throw new Error(
    `Cannot determine index stride: ${count} entries in ${payload} bytes ` +
    `implies ${(payload / count).toFixed(2)} bytes each, and neither ` +
    `${INDEX_ENTRY_V1} nor ${INDEX_ENTRY_V2} yields an ascending cell order.`
  );
}

/** Do the first entries read at `stride` have strictly ascending h3 cells? */
function _entriesLookSorted(section, count, stride) {
  const n = Math.min(count, 16);
  if (n < 2) return section.length >= 4 + stride;
  let prev = -1n;
  for (let i = 0; i < n; i++) {
    const o = 4 + i * stride;
    if (o + 8 > section.length) return false;
    const cell = readPacked(section, o, 8, true);
    if (cell === 0n || cell <= prev) return false;
    prev = cell;
  }
  return true;
}

export function readIndex(sl) {
  const dv = new DataView(sl.buffer, sl.byteOffset, sl.length);
  const cnt = u32(dv, 0);
  const mask7 = BigInt('0xfffffffff8000000');
  const stride = indexStride(sl.length, cnt, sl);
  const e = [], m = new Map();
  if (stride === INDEX_ENTRY_V2) {
    let o = 4;
    for (let i = 0; i < cnt; i++) {
      const cb = readPacked(sl, o, 8, true);
      // 8 cell + 16 bbox + 6 offset_lo + 2 len_lo + 1 offset_hi + 1 len_hi
      // + 2 feature_count + 2 cell_index = 38.
      e.push({ h3Cell: cb, minLon: i32(dv, o + 8), minLat: i32(dv, o + 12), maxLon: i32(dv, o + 16), maxLat: i32(dv, o + 20),
        blockOffset: readPacked(sl, o + 24, 6, true) | (BigInt(sl[o + 32]) << 48n),
        blockLength: readPacked(sl, o + 30, 2, false) | (sl[o + 33] << 16),
        featureCount: u16(dv, o + 34), cellIndexInBlock: u16(dv, o + 36) });
      m.set(cb & mask7, i); o += INDEX_ENTRY_V2;
    }
  } else {
    let o = 4;
    for (let i = 0; i < cnt; i++) {
      ...
      m.set(cb & mask7, i); o += INDEX_ENTRY_V1;
    }
  }
  // Bytes the index section carries beyond what its entries need. Non-zero
  // only for the published files whose index_length was computed at 42 bytes
  // against 38-byte entries; there, blocks_offset and every absolute block
  // offset were derived from the inflated length and overshoot by exactly
  // this much, so the reader subtracts it back off.
  const slack = (sl.length - 4) - cnt * stride;
  return { entries: e, cellMap: m, stride, slack };
}
...
const STATE_FILE_RE = new RegExp(`^([A-Z]{2})\\.(?:${LAYER_SUFFIXES.buildings.join('|')})\\.ptiles$`);
...
class _PtilesFile {
  constructor(data, header, index, dict) {
    ...
    // Absolute offsets in a slack-bearing file are shifted by the overshoot.
    // Relative ones are not: they are measured from blocks_offset, which moves
    // with the same error and cancels out.
    this._offsetFix = this._relOff ? 0 : (index.slack || 0);
  }
  ...
  async decompressBlock(entry) {
    ...
    const abs = this._relOff ? this._h.blocksOffset + bo : bo - this._offsetFix;
    const blob = this.data.subarray(abs, abs + Number(entry.blockLength));
    const hasDict = this._dict.length > 0;
    const dec = await ensureZstd(hasDict);
    return hasDict ? dec.decompressWithDict(blob, this._dict) : dec.decompress(blob);
  }
  ...
  async _getReader(lat, lon) {
    ...
    if (this._baseUrl) {
      ...
      if (!this._files.has(abbr)) {
        ...
        // Try the published name first, then older builds.
        let loaded = null;
        for (const suffix of (this._suffixes || LAYER_SUFFIXES.buildings)) {
          const resp = await fetch(`${base}${abbr}.${suffix}.ptiles`);
          if (resp.ok) { loaded = await loadFile(new Uint8Array(await resp.arrayBuffer())); break; }
        }
        if (!loaded) return null;
        this._files.set(abbr, loaded);
      }
