 ...
 */
export function indexStride(byteLength, count) {
  ...
  throw new Error(
    ...
    `implies ${(payload / count).toFixed(2)} bytes each, expected ` +
    `${INDEX_ENTRY_V1} or ${INDEX_ENTRY_V2}.`
  );
}
...
export function readIndex(sl) {
  ...
  const stride = indexStride(sl.length, cnt);
  const e = [], m = new Map();
  ...
  return { entries: e, cellMap: m, stride };
}
