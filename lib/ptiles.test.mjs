...
describe('index stride', () => {
  ...
  test('a short section, too small for either width, throws', () => {
    ...
    assert.throws(() => indexStride(4 + 10 * 12, 10), /Cannot determine index stride/);
  });

  test('the error names what it saw', () => {
    assert.throws(() => indexStride(4 + 10 * 12, 10), /12\.00 bytes each/);
  });

  test('a section longer than its entries need is probed, not rejected', () => {
    // US.signals and US.camera shipped with index_length computed at 42 bytes
    // while the encoder emitted 38 — 432,692 bytes of slack on signals. Those
    // files are on the CDN, so refusing them would refuse real data.
    assert.equal(indexStride(4 + 48 * 42, 48), INDEX_ENTRY_V2);
  });
...
describe('readIndex', () => {
  ...
  test('a section too small for any known width throws', () => {
    assert.throws(() => readIndex(makeIndex(4, 12)), /index stride/);
  });

  test('probing rejects a width that leaves cells out of order', () => {
    // Entries are sorted by h3_cell, so reading at the wrong width scrambles
    // them almost immediately. That is what makes probing safe rather than a
    // coin flip: 48 entries of zeroes cannot pass at any width.
    const idx = makeIndex(48, 42);
    assert.throws(() => readIndex(idx), /ascending cell order/);
  });
});
 ...
 *
 * The intended source is the conformance corpus in the ptile-client repo
 * (conformance/corpus): real published bytes sliced down, keeping the real
 * header, index entries and block payloads. It deliberately includes
 * US.signals.stride42 and US.camera.stride42 — the published files whose
 * index_length was computed at 42 bytes while entries were emitted at 38.
 *
 *   PTILES_FIXTURES=../ptile-client/conformance/corpus node --test lib/ptiles.test.mjs
 */
describe('real files', { skip: !process.env.PTILES_FIXTURES && 'set PTILES_FIXTURES' }, () => {
  ...
  async function openFirstBlock(file) {
    ...
    // Same resolution the reader uses: relative offsets are measured from
    // blocks_offset, absolute ones are shifted back by any index slack.
    const rec = Number(e.blockOffset);
    const relative = rec < blocksOff;
    const off = relative ? blocksOff + rec : rec - (idx.slack || 0);
  ...
  test('the 42-vs-38 published skew is recovered, not refused', async (t) => {
    const skewed = files.filter(f => f.includes('stride42'));
    if (!skewed.length) return t.skip('corpus has no stride42 case');
    for (const f of skewed) {
      ...
      const idxOff = dv.getUint32(52, true), idxLen = dv.getUint32(60, true);
      const idx = readIndex(d.subarray(idxOff, idxOff + idxLen));
      // Entries are 38 bytes even though index_length was sized for 42, so the
      // section carries slack. Refusing it would refuse files on the CDN.
      assert.equal(idx.stride, INDEX_ENTRY_V2, `${f}: probed the wrong width`);
      const slack = (idxLen - 4) - idx.entries.length * idx.stride;
      assert.ok(slack > 0, `${f}: expected trailing slack`);
      assert.equal(slack, 192, `${f}: manifest records overshoot 192`);
    }
  });

  test('a slack-bearing file has its absolute block offsets corrected', async (t) => {
    const skewed = files.filter(f => f.includes('stride42'));
    if (!skewed.length) return t.skip('corpus has no stride42 case');
    for (const f of skewed) {
      // The block is only reachable after subtracting the overshoot; reading
      // the recorded offset verbatim lands mid-index and zstd rejects it.
      const { raw } = await openFirstBlock(f);
      assert.ok(raw.length > 0, `${f}: corrected offset did not reach a block`);
    }
  });

  test('cells come back ascending, which is what makes probing sound', async () => {
    for (const f of files) {
      const buf = fs.readFileSync(path.join(dir, f));
      const d = new Uint8Array(buf.buffer, buf.byteOffset, buf.byteLength);
      const dv = new DataView(d.buffer, d.byteOffset, 256);
      const idxOff = dv.getUint32(52, true), idxLen = dv.getUint32(60, true);
      const { entries } = readIndex(d.subarray(idxOff, idxOff + idxLen));
      for (let i = 1; i < entries.length; i++) {
        assert.ok(entries[i].h3Cell > entries[i - 1].h3Cell,
          `${f}: entry ${i} breaks cell ordering`);
      }
    }
  });
});
