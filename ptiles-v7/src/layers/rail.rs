/// Rail layer record (LineString geometry: railways, subways, trams, etc.)
///
/// Binary layout:
///   osm_id: varint (delta)
///   vertex_count: uint8
...
///   flags: uint8
///   rail_type_idx: uint8
///   [rail_type_str]: uint8 len + UTF-8 (if rail_type_idx == 255)
///   [name]: uint16 len + UTF-8 (if flags & 0x01)

use crate::varint;

pub static RAIL_TYPE_REVERSE: [&str; 11] = [
    "rail", "light_rail", "subway", "tram", "monorail",
    "narrow_gauge", "funicular", "disused", "abandoned",
    "preserved", "miniature",
];

pub fn rail_type_index(s: &str) -> u8 {
    RAIL_TYPE_REVERSE.iter().position(|&t| t == s).map(|i| i as u8).unwrap_or(255)
}
...
impl RailRecord {
    pub fn decode(data: &[u8], offset: usize, prev_osm_id: u64) -> (Self, usize, u64) {
        ...
        pos += 1;
        let rail_type = if rtype_idx == 255 {
            ...
            RAIL_TYPE_REVERSE.get(rtype_idx as usize).unwrap_or(&"rail").to_string()
        };
